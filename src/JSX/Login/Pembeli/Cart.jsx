// src\JSX\Login\Pembeli\Cart.jsx (Modified)
...
const Cart = () => {
    ...
    // ... (handleLogout function - tetap sama) ...
    const handleLogout = () => {
    ...
    const totalCartPrice = cart.items.reduce((sum, item) => sum + (1 * (item.barang?.harga_barang || 0)), 0); // Kuantitas selalu 1
    ...
    return (
        <div className="cart-page">
            ...
            <div className="cart-container">
                ...
                {cart.items.length === 0 ? (
                    <p className="empty-cart-message">Keranjang Anda kosong. Yuk, <Link to="/shop-pembeli">mulai belanja!</Link></p>
                ) : (
                    <div className="cart-items-list">
                        {cart.items.map(item => (
                            <div className="cart-item-card" key={item.id_cart_item}>
                                <div className="item-info">
                                    <img src={`http://127.0.0.1:8000/images/${item.barang?.image}`} alt={item.barang?.nama_barang || 'Barang'} className="item-image" />
                                    <div className="item-details">
                                        <h3>{item.barang?.nama_barang || 'Barang Tidak Dikenal'}</h3>
                                        <p className="item-price">Rp {Number(item.barang?.harga_barang || 0).toLocaleString('id-ID')}</p>
                                        <p className="item-quantity">Kuantitas: 1</p>
                                    </div>
                                </div>
                ...
                {cart.items.length > 0 && (
                    <div className="cart-summary">
                        ...
                        {/* UBAH TOMBOL INI KE LINK ATAU BUTTON UNTUK NAVIGASI KE HALAMAN CHECKOUT */}
                        <button className="checkout-btn" onClick={() => navigate('/checkout')}> {/* <-- UBAH DI SINI */}
